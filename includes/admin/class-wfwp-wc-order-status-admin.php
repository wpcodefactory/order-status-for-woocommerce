...
if ( ! class_exists( 'WFWP_WC_Order_Status_Admin' ) ) :
	...
	/**
	 * WFWP_WC_Order_Status_Admin class.
	 *
	 * @version 2.0.0
	 * @since   1.0.0
	 */
	class WFWP_WC_Order_Status_Admin {
		...
		public function __construct() {
			...
			add_action( 'save_post_wfwp_wc_order_status', array( $this, 'save_meta_box' ), PHP_INT_MAX );

			add_filter( 'wp_insert_post_data', array( $this, 'on_insert_post_data' ), PHP_INT_MAX );
		 ...
		 */
		public function filter_options( $options, $post_id = false ) {
			...
			if ( $status ) {
				...
				if ( $status->is_override() ) {
					...
					if ( isset( $options['order_options']['options']['do_download_permissions'] ) ) {
						unset( $options['order_options']['options']['do_download_permissions'] );
					}
		 ...
		 */
		public function get_options( $post_id = false ) {
		...
		public function enqueue_scripts() {
			...
			if (
				...
				wp_enqueue_script(
					'wfwp-wcos-admin',
					wfwp_wc_order_status()->plugin_url() . '/assets/js/wfwp-wcos-admin' . $min . '.js',
					array( 'jquery' ),
		 ...
		 */
		public function delete_status_notices() {
		 ...
		 */
		public function on_insert_post_data( $data ) {
		 ...
		 * @since   1.0.0
		 *
		 * @todo (v2.0.0) screen: double-check?
		...
		public function enqueue_styles() {
			...
			$is_custom_status_screen = (
				'wfwp_wc_order_status' === $screen->post_type &&
				in_array( $screen->base, array( 'edit', 'post' ), true )
			);

			$is_order_screen = in_array(
				$screen->id,
				array(
					'edit-shop_order',                     // Legacy orders list.
					'shop_order',                          // Legacy order edit.
					wc_get_page_screen_id( 'shop-order' ), // HPOS orders.
				),
				true
			);

			if ( ! $is_custom_status_screen && ! $is_order_screen ) {
				return;
			}
			...
			wp_enqueue_style(
				'wfwp-wcos-admin',
				wfwp_wc_order_status()->plugin_url() . '/assets/css/wfwp-wcos-admin' . $min . '.css',
				array(),
				wfwp_wc_order_status()->version
			);

			$style    = '';
			$statuses = wfwp_wc_order_status()->core->get_statuses();
			foreach ( $statuses as $status ) {
				// Status.
				$style .= 'mark.order-status.status-' . sanitize_html_class( $status->slug ) . ' {
					color: ' . sanitize_hex_color( $status->text_color ) . ' !important;
					background-color: ' . sanitize_hex_color( $status->bg_color ) . ' !important;
				}' . PHP_EOL;
				...
				$style .= '.view.' . sanitize_html_class( $status->slug ) . '::after {
					font-family: WooCommerce !important;
					color: ' . sanitize_hex_color( $status->order_list_icon_color ) . ' !important;
					background-color: ' . sanitize_hex_color( $status->order_list_icon_bg_color ) . ' !important;
					content: "\\' . preg_replace( '/[^a-fA-F0-9]/', '', $status->order_list_icon ) . '" !important;
				}' . PHP_EOL;
			}
			if ( ! empty( $style ) ) {
				wp_add_inline_style( 'wfwp-wcos-admin', $style );
			}
		}
		 ...
		 * @param array   $metabox Meta box array.
		 *
		...
		public function meta_box_callback( $post, $metabox ) {
			...
			if ( ! empty( $options[ $metabox['args']['id'] ]['options'] ) ) {
				...
				foreach ( $options[ $metabox['args']['id'] ]['options'] as $option ) {
					...
					if ( 'title' === $option['type'] ) {
						...
						} elseif ( get_post_meta( $post_id, $meta_name, false ) ) {
			...
			echo wp_kses(
				$html,
				$this->get_meta_box_allowed_html()
			);

			if (
				isset( $metabox['args']['id'] ) &&
				'main_data' === $metabox['args']['id']
			) {
				wp_nonce_field(
					'wfwp_wc_order_status_save',
					'_wfwp_wc_order_status_save_nonce'
				);
			}
		}

		/**
		 * Get meta box allowed html.
		 *
		 * @version 2.0.0
		 * @since   2.0.0
		 *
		 * @todo (v2.0.0) allowed html: double-check?
		 */
		public function get_meta_box_allowed_html() {
			$allowed_html = array(
				'input'  => array(
					'type'        => true,
					'id'          => true,
			...
			$allowed_post_html['a']['onclick'] = true;
		 ...
		 *
		 * @todo (v2.0.0) sanitize: double-check?
		 */
		public function save_meta_box( $post_id ) {
			if (
				! isset( $_POST['_wfwp_wc_order_status_save_nonce'] ) ||
				! wp_verify_nonce(
					sanitize_text_field( wp_unslash( $_POST['_wfwp_wc_order_status_save_nonce'] ) ),
					'wfwp_wc_order_status_save'
				)
			) {
				return;
				...
				foreach ( $section['options'] as $option ) {
					...
					update_post_meta(
						...
						(
							...
							sanitize_textarea_field( wp_unslash( $_POST[ $option['id'] ] ) ) :
							$option['default']
						)
					);
				}
