...
class WFWP_WC_Shop_Order_Status {
	...
	function __construct( $post_id = false ) {
		...
		$default_subject                 = sprintf( __( '%s Order %s status changed to %s - %s', 'order-status-for-woocommerce' ),
			'[{site_title}]', '#{order_number}', '{status_to_title}', '{order_date}' );
		$default_heading                 = sprintf( __( 'Order status changed to %s', 'order-status-for-woocommerce' ),
			'{status_to_title}' );
		$default_content                 = sprintf( __( 'Order %s status changed from %s to %s', 'order-status-for-woocommerce' ),
			'#{order_number}', '{status_from_title}', '{status_to_title}' );
